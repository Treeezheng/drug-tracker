...
type Action='username'|'account'|'recovery'|'sessions'|'device';
const titles:Record<Action,string>={username:'Check username',account:'Change password',recovery:'Replace recovery key',sessions:'Sign out all devices',device:'Automatically unlock this device'};
export default function CloudSecuritySettings({ownerId,username}:{ownerId:string;username?:string}){
  ...
  async function submit(event:React.FormEvent<HTMLFormElement>){
    event.preventDefault();if(!action||action==='username'||inFlight.current)return;
  ...
  return <><details className="privacy-details security-settings"><summary>Security</summary><p className="muted">Sign-in and unlocking last up to 7 days. Lock or sign out to require your password sooner.</p><div className="setting-with-help"><label className="check-line"><input type="checkbox" checked={preference.enabled} disabled={loadingPreference||busy} onChange={event=>{setError('');setNotice('');if(event.target.checked)setAction('device');else void disableAutoUnlock();}}/>Auto-unlock on this device</label><SettingHelp label="automatic unlocking">Keeps an encrypted unlocking key only in this browser for up to 7 days. Anyone who can use this browser can open your records during that time. Visiting again does not extend the deadline. Lock, sign out, or turn this off to remove the saved unlocking key.</SettingHelp></div>{preference.enabled&&preference.expiresAt&&<p className="field-hint">Until {new Date(preference.expiresAt).toLocaleString()}. Your password is not stored.</p>}{!action&&error&&<p className="inline-error" role="alert">{error}</p>}{!action&&notice&&<p className="field-hint" role="status">{notice}</p>}<div className="security-actions">{(['username','account','recovery','sessions'] as Action[]).map(value=><button className="button secondary small" key={value} onClick={()=>{setNotice('');setError('');setAction(value);}}>{titles[value]}</button>)}</div></details>
    {action&&<Modal title={recoveryKey?'Save your new recovery key':titles[action]} onClose={close} closeDisabled={Boolean(recoveryKey)}>{action==='username'?<>{username?<label className="field"><span>Username</span><input type="text" value={username} readOnly autoComplete="off"/></label>:<p className="muted">Your username is unavailable for this session.</p>}<div className="modal-footer"><button type="button" className="button primary" onClick={close}>Done</button></div></>:recoveryKey?<RecoveryKeyPanel recoveryKey={recoveryKey} rotated onDone={close}/>:notice?<><p role="status">{notice}</p><button className="button primary" onClick={close}>Done</button></>:<form key={action} name={`security-${action}`} method="post" className="auth-form" onSubmit={submit}>
