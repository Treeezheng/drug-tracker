...
import { Pill, Plus, Check, ChevronLeft, ChevronRight, CalendarDays } from 'lucide-react';
...
export default function GuestSimulator({onSignIn,onRegister,notice=''}:{onSignIn:()=>void;onRegister:()=>void;notice?:string}){
  ...
  const {profile,drafts,favorites,date,days,publishedOnly}=workspace;
  ...
  function editDose(previous:Dose,next:Dose){
    const preferred=favorites.find(f=>f.productId===next.productId);
    const updated=previous.productId!==next.productId&&preferred?updateDose(next,{packageStrength:preferred.packageStrength||preferred.strength,quantity:preferred.quantity},profile.timeZone):next;
    change({drafts:drafts.map(d=>d.id===previous.id?{...updated,status:'simulated'}:d)});
  }
  function clear(){
  ...
  return <div className="simple-shell guest-shell"><header className="app-header"><a href="#" className="brand" onClick={event=>{event.preventDefault();setPage('simulation');}}><Pill size={22}/><span>Drug Tracker</span></a><nav className="app-nav" aria-label="Main navigation"><button className={page==='simulation'?'active':''} aria-current={page==='simulation'?'page':undefined} onClick={()=>setPage('simulation')}>Dose Simulation</button><button onClick={onSignIn}>History</button><button className={page==='settings'?'active':''} aria-current={page==='settings'?'page':undefined} onClick={()=>setPage('settings')}>Settings</button></nav><div className="account-control"><button className="text-button" onClick={onSignIn}>Sign in</button><button className="button secondary small" onClick={onRegister}>Create account</button></div></header>
    <main><div className="page-title"><h1>{page==='simulation'?'Dose Simulation':'Simulation settings'}</h1></div><p className="guest-storage-note">Guest simulation · Stored unencrypted in this browser when available. Account records are separate.</p>
      ...
      <section className="card dose-card"><div className="section-heading"><h2>Doses</h2><button className="text-button" onClick={()=>setChoosing(true)}>My medications</button></div>{drafts.map((dose,index)=><div className="planned-row" key={dose.id}><DoseEditor dose={dose} index={index} profile={profile} productIds={favorites.map(f=>f.productId)} onChange={next=>editDose(dose,next)} onRemove={()=>change({drafts:drafts.filter(d=>d.id!==dose.id)})} onDuplicate={()=>{if(drafts.length<100)change({drafts:[...drafts,{...dose,id:crypto.randomUUID(),status:'simulated'}]});else setError('This guest workspace supports up to 100 simulated doses.');}}/><div className="row-state"><span className="muted">Simulated</span><button className="button secondary small" onClick={onSignIn}><Check size={15}/>Taken</button></div></div>)}{!drafts.length&&<div className="empty-state">{favorites.length?'Add a dose to explore its timeline.':'Choose My medications to start a simulation.'}</div>}<div className="dose-actions"><button className="button secondary desktop-add-dose" onClick={()=>addDose()}><Plus size={16}/>Add dose</button>{favorites.map(f=><button className="favorite-chip" key={f.id} onClick={()=>addDose(f)}><Plus size={13}/><MedicationName id={f.productId} name={products.find(p=>p.id===f.productId)?.name||f.productId}/> {f.packageStrength||f.strength} {products.find(p=>p.id===f.productId)?.strengthUnit||''}</button>)}</div></section>
      <details className="advanced-options"><summary>Simulation options</summary><div className="advanced-content"><label className="check-line"><input type="checkbox" checked={publishedOnly} onChange={e=>change({publishedOnly:e.target.checked})}/>Published trace only (no estimated Concerta tail)</label></div></details>{scoped.sourceIds.length>0&&<details className="page-references"><summary>References</summary>{sources.filter(s=>scoped.sourceIds.includes(s.id)).map(s=><a key={s.id} href={s.url} target="_blank" rel="noreferrer">{s.title}<small>{s.section}</small></a>)}</details>}</>:<><section className="card settings-section"><div className="section-heading"><h2>My medications</h2><button className="button secondary small" onClick={()=>setChoosing(true)}>Choose medications</button></div><p className="muted">These choices are for this browser’s guest simulation.</p></section><form className="card settings-section guest-preferences" onSubmit={event=>{event.preventDefault();try{guestDayWindow(date,days,preferences.timeZone);sleepIntervals(Date.now(),Date.now()+86400000,preferences);change({profile:{...preferences,name:''}});}catch(cause){setError((cause as Error).message);}}}><h2>Time & sleep</h2><div className="settings-grid"><label className="field"><span>Time zone</span><input value={preferences.timeZone} onChange={e=>setPreferences(p=>({...p,timeZone:e.target.value}))} required maxLength={80}/></label><label className="field"><span>Time display</span><select value={preferences.timeFormat} onChange={e=>setPreferences(p=>({...p,timeFormat:e.target.value as '12h'|'24h'}))}><option value="12h">12-hour</option><option value="24h">24-hour</option></select></label><label className="field"><span>Minute increment</span><select value={preferences.timeIncrementMinutes||5} onChange={e=>setPreferences(p=>({...p,timeIncrementMinutes:Number(e.target.value) as 5|10}))}><option value="5">5 minutes</option><option value="10">10 minutes</option></select></label></div><label className="check-line"><input type="checkbox" checked={preferences.sleepEnabled} onChange={e=>setPreferences(p=>({...p,sleepEnabled:e.target.checked}))}/>Show a sleep window</label>{preferences.sleepEnabled&&<div className="settings-grid"><label className="field"><span>Bedtime</span><input type="time" required value={preferences.bedtime} onChange={e=>setPreferences(p=>({...p,bedtime:e.target.value}))}/></label><label className="field"><span>Wake time</span><input type="time" required value={preferences.wakeTime} onChange={e=>setPreferences(p=>({...p,wakeTime:e.target.value}))}/></label></div>}<button className="button secondary" type="submit">Save simulation settings</button></form></>}
    ...
    </main><footer className="app-footer"><span>Open source · Built with OpenAI Codex (GPT-6)</span><a href="https://github.com/Treeezheng/drug-tracker" target="_blank" rel="noreferrer">GitHub</a><a href={`${import.meta.env.BASE_URL}privacy.html`} target="_blank" rel="noreferrer">Privacy</a></footer>
