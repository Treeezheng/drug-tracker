...
export function currentDoseTime(zone:string,increment:5|10,at=Date.now()):{date:string;time:string;administeredAt:string;disambiguation?:Dose['disambiguation']} {
  if(![5,10].includes(increment)||!Number.isFinite(at))throw new RangeError('Choose a valid time and a 5 or 10 minute increment.');
...
export default function DoseEditor({dose,index,profile,onChange,onRemove,onDuplicate,actual,productIds}:Props){
  ...
  const illustrative=!modelGroup(dose).reference,inputError=dose.productId?doseInputError(dose):'';
  ...
  return <div className={`dose-editor ${!dose.administeredAt?'pending':''}`}>
    ...
    <label className="field medication-field"><span>Medication</span><select aria-label={`Dose ${index+1} medication`} value={dose.productId} onChange={e=>update({productId:e.target.value})}><option value="">Choose medication</option>{options.map(item=><option key={item.id} value={item.id}>{item.id===dose.productId&&dose.productName?dose.productName:item.name} · {item.id===dose.productId&&dose.formulation?dose.formulation:item.formulation}</option>)}{historical&&<option value={dose.productId}>{dose.productName||'Historical medication'} · {dose.formulation||'Formulation not recorded'}</option>}</select></label>
