...
import { isSymptomCheckin, latestCheckins, makeSymptomCheckin, SYMPTOMS, SYMPTOM_LABELS, symptomDraftFromCheckin, toggleSymptom } from '../lib/symptoms';
  ...
  return <form className="symptom-form" noValidate onSubmit={async event => {
    ...
    <fieldset disabled={busy}>
      ...
      <div className="symptom-chips" role="group" aria-label="Discomfort symptoms">
        {SYMPTOMS.map(symptom => <button key={symptom.id} type="button" className={`symptom-chip ${draft.symptoms.includes(symptom.id) ? 'selected' : ''}`} aria-pressed={draft.symptoms.includes(symptom.id)} onClick={() => setDraft(previous => ({ ...previous, symptoms: toggleSymptom(previous.symptoms, symptom.id) }))}>{draft.symptoms.includes(symptom.id) && <Check size={13} aria-hidden="true" />}{symptom.label}</button>)}
      </div>
      <details className="symptom-time-details"><summary>Date & time</summary><div className="symptom-time-row">
        ...
        <label className="field"><span>Time</span><input type="time" step={(profile.timeIncrementMinutes ?? 5) * 60} aria-label="Discomfort time" aria-invalid={!!timeError} aria-describedby={timeError ? timeErrorId : undefined} value={draft.time} onChange={event => { setCustomTime(true); setDraft(previous => ({ ...previous, time: event.target.value, disambiguation: undefined })); }} /></label>
...
export function SymptomRecords({ entries, profile, onSave, onRemove }: { entries: Checkin[]; profile: Profile; onSave?: SaveCheckin; onRemove?: RemoveCheckin }) {
  ...
  return <>
    ...
    <div className="symptom-records">{shown.map(entry => <div className="symptom-record" key={entry.id}>
      <div className="symptom-record-copy"><time dateTime={entry.recordedAt}>{formatInstant(Date.parse(entry.recordedAt!), profile)}</time><p>{entry.symptoms!.map(id => SYMPTOM_LABELS[id] || id).join(' · ')}</p>{entry.note && <details><summary>Note</summary><p className="symptom-saved-note">{entry.note}</p></details>}</div>
      <div className="symptom-record-actions">{onSave && <button type="button" className="icon-button" disabled={!!busy} aria-label={`Edit discomfort check-in ${formatInstant(Date.parse(entry.recordedAt!), profile)}`} onClick={() => setEditing(entry)}><Pencil size={15}/></button>}{onRemove && <button type="button" className="icon-button" disabled={!!busy} aria-label={`Delete discomfort check-in ${formatInstant(Date.parse(entry.recordedAt!), profile)}`} onClick={async () => {
...
export default function Symptoms({ checkins, profile, onSave, onRemove }: { checkins: Checkin[]; profile: Profile; onSave: SaveCheckin; onRemove: RemoveCheckin }) {
  const entries = latestCheckins(checkins).filter(isSymptomCheckin).sort((a, b) => Date.parse(b.recordedAt!) - Date.parse(a.recordedAt!));
