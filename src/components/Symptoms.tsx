...
import { isSymptomCheckin, latestCheckins, makeSymptomCheckin, PRIMARY_SYMPTOM_IDS, POSITIVE_SYMPTOM_IDS, SYMPTOMS, SYMPTOM_LABELS, symptomDraftFromCheckin, toggleSymptom } from '../lib/symptoms';
...
export function SymptomForm({ profile, entry, onSave, onSaved }: { profile: Profile; entry?: Checkin; onSave: SaveCheckin; onSaved?: () => void }) {
  ...
  const chip=(symptom:typeof SYMPTOMS[number])=><button key={symptom.id} type="button" className={`symptom-chip${POSITIVE_SYMPTOM_IDS.includes(symptom.id) ? ' positive' : ''} ${draft.symptoms.includes(symptom.id) ? 'selected' : ''}`} aria-pressed={draft.symptoms.includes(symptom.id)} onClick={() => setDraft(previous => ({ ...previous, symptoms: toggleSymptom(previous.symptoms, symptom.id) }))}>{draft.symptoms.includes(symptom.id) && <Check size={13} aria-hidden="true" />}{symptom.label}</button>;
  return <form className="symptom-form" noValidate onSubmit={async event => {
    ...
    <fieldset disabled={busy}>
      <legend className="sr-only">Feeling / discomfort check-in</legend>
      <div className="symptom-chips" role="group" aria-label="Feeling / discomfort choices">
        {PRIMARY_SYMPTOM_IDS.map(id=>chip(SYMPTOMS.find(item=>item.id===id)!))}
      </div>
      <details className="symptom-more" open={entry?.symptoms?.some(id=>!PRIMARY_SYMPTOM_IDS.includes(id as typeof PRIMARY_SYMPTOM_IDS[number]))||undefined}><summary>More symptoms</summary><div className="symptom-chips" role="group" aria-label="Other feelings and symptoms">{SYMPTOMS.filter(item=>!PRIMARY_SYMPTOM_IDS.includes(item.id)).map(chip)}</div></details>
      <details className="symptom-time-details"><summary>Date & time</summary><div className="symptom-time-row">
        <label className="field"><span>Date</span><input type="date" aria-label="Feeling / discomfort date" aria-invalid={!!timeError} aria-describedby={timeError ? timeErrorId : undefined} value={draft.date} onChange={event => { setCustomTime(true); setDraft(previous => ({ ...previous, date: event.target.value, disambiguation: undefined })); }} /></label>
        <div className="field"><span>Time</span><MobileTimePicker minuteStep={profile.timeIncrementMinutes??5} timeFormat={profile.timeFormat} label="Feeling / discomfort time" invalid={!!timeError} describedBy={timeError?timeErrorId:undefined} value={draft.time} onChange={time => { setCustomTime(true); setDraft(previous => ({ ...previous, time, disambiguation: undefined })); }}/></div>
        <button type="button" className="button subtle" aria-label="Use current time for feeling / discomfort" onClick={() => { setCustomTime(false); setDraft(previous => ({ ...previous, ...currentDoseTime(profile.timeZone, profile.timeIncrementMinutes ?? 5) })); }}>Now</button>
      </div>
      {repeatedTime && <label className="field"><span>Clock occurrence</span><select aria-label="Feeling / discomfort clock occurrence" value={draft.disambiguation ?? ''} onChange={event => setDraft(previous => ({ ...previous, disambiguation: event.target.value as 'earlier' | 'later' || undefined }))}><option value="">Choose occurrence</option><option value="earlier">Earlier occurrence</option><option value="later">Later occurrence</option></select></label>}
      {timeError && <p id={timeErrorId} className="inline-error" role="alert">{timeError}</p>}</details>
      <details className="symptom-note"><summary>Note (optional)</summary><label className="field"><span className="sr-only">Feeling / discomfort note</span><textarea aria-label="Feeling / discomfort note (optional)" maxLength={2000} rows={2} value={draft.note} onChange={event => setDraft(previous => ({ ...previous, note: event.target.value }))} /></label></details>
...
export function SymptomRecords({ entries, profile, onSave, onRemove }: { entries: Checkin[]; profile: Profile; onSave?: SaveCheckin; onRemove?: RemoveCheckin }) {
  ...
  return <>
    ...
    <div className="symptom-records">{shown.map(entry => {
      ...
      <div className="symptom-record-actions">{onSave && isSymptomCheckin(entry) && <button type="button" className="icon-button" disabled={!!busy} aria-label={`Edit feeling / discomfort check-in ${label}`} onClick={() => setEditing(entry)}><Pencil size={15}/></button>}{onRemove && <button type="button" className="icon-button" disabled={!!busy} aria-label={`Delete feeling / discomfort check-in ${label}`} onClick={async () => {
        if (!window.confirm('Delete this feeling / discomfort check-in?')) return;
    ...
    {editing && onSave && <Modal title="Edit feeling / discomfort" onClose={() => setEditing(null)}><SymptomForm key={`${editing.id}-${editing.revision ?? 0}-${profile.timeZone}`} profile={profile} entry={editing} onSave={onSave} onSaved={() => setEditing(null)}/></Modal>}
  </>;
}
