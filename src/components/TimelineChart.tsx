...
export default function TimelineChart({doses,date,days,profile,publishedOnly,onMove,onProfile,baseline='empty',hasPendingDose=false,omittedHistoryCount=0,omittedUnknownHistoryCount=0,onAddDose,onSources}:Props){
  ...
  }),[groups,eventRows,visible,sampleTimes,publishedOnly,start,end]);
  ...
  return <div className="simple-timeline"><div className="chart-main" ref={ref}>
    {visible.length===0?<TimelineEmptyState pending={hasPendingDose||doses.some(d=>d.status==='simulated')} hasHistory={omittedHistoryCount>0||omittedUnknownHistoryCount>0||doses.some(d=>d.status==='actual')} onAddDose={onAddDose}/>:plots.map(({group,timing,members,ceiling,totalPaths,curves,dataNote},index)=>{
