...
export default function TimelineChart({doses,date,days,profile,publishedOnly,onMove,onProfile,baseline='empty'}:Props){
  ...
  return <div className="simple-timeline"><div className="chart-main" ref={ref}>
    {groups.map((group,index)=>{
      ...
      {members.length>0&&<div className="chart-legend">{!timing&&<span><i style={{background:'#426a95'}}/>{current?.complete?(unit==='ng/mL'?'Modeled total':'Illustrative sum'):known(current,at)?'Known contributions · incomplete':'Unknown total'}</span>}{timelineLegend(members).map(({key,dose:d,members:entries})=><span className="legend-medication" key={key}><span className="legend-swatches" aria-hidden="true">{entries.map(entry=><i key={entry.id} style={{background:colors[visible.findIndex(v=>v.id===entry.id)%colors.length]}}/>)}</span><span className="legend-label"><MedicationName id={d.productId} name={d.productName}/><span className="legend-strength">{d.packageStrength||d.strength} {d.strengthUnit||'mg'}</span></span></span>)}</div>}
      </div>;
    })}
    {names.length>0&&<div className="reading-control"><span>{formatInstant(at,profile,true)}</span><div className="reading-values">{Object.entries(totals).filter(([group])=>names.includes(group)).map(([group,g])=><span key={group} className="reading-value">{names.length>1&&<small>{group}</small>}<span><strong>{known(g,at)?g.value.toFixed(2):'—'}</strong> <small>{g.unit}</small></span>{!g.complete&&<small>Incomplete</small>}</span>)}</div>{pinned!==null&&<button className="text-button" onClick={()=>setPinned(null)}>Unpin</button>}</div>}
  </div>
