...
import { hasKnownTotal, hasMissingTimelineData, timelineReading } from '../lib/timeline-data';
import { useEffect, useId, useMemo, useRef, useState } from 'react';
import { ChevronRight } from 'lucide-react';
import type { Dose, Profile } from '../lib/types';
import { concentrationAnalyte, contributionForGroup, contributions, contributesToGroup, doseTimestamp, effectWindow, groupedTotals, includeTimelineDose, modelGroup, referenceForDose, referenceOverlay } from '../lib/model';
...
export default function TimelineChart({doses,date,days,profile,publishedOnly,onMove,onProfile,baseline='empty',hasPendingDose=false,omittedHistoryCount=0,omittedUnknownHistoryCount=0,onAddDose,onSources}:Props){
  const readingsId=useId(),[showReadings,setShowReadings]=useState(false);
  ...
  return <div className="simple-timeline"><div className="chart-main" ref={ref}>
    {visible.length===0?<TimelineEmptyState pending={hasPendingDose||doses.some(d=>d.status==='simulated')} hasHistory={omittedHistoryCount>0||omittedUnknownHistoryCount>0||doses.some(d=>d.status==='actual')} onAddDose={onAddDose}/>:groups.map((group,index)=>{
      ...
      const referenceOnly=overlays.length>0&&members.every(dose=>!modelGroup(dose).reference&&!dose.assumptions?.accepted);
      ...
      for(let i=1;i<series.length;i++){
        ...
        const kind=before!.complete&&after!.complete&&!before!.tail&&!after!.tail?'solid':'estimated';
      ...
      const missingData=hasMissingTimelineData(members,start,end,publishedOnly,timing?undefined:group);
      return <div className="analyte-panel" key={group}><div className="chart-heading"><h2>{group}</h2><div className="chart-heading-meta"><span className="muted">{timing?'':`${unit} · estimate`}</span><span className="chart-simulation-note">Simulation only · Not medical advice</span></div></div><div className="day-headings" style={{paddingLeft:L,paddingRight:R}}>{headers.map(h=><span key={h.date} style={{width:`${(h.end-h.start)/(end-start)*100}%`}}>{new Intl.DateTimeFormat('en-US',{timeZone:profile.timeZone,weekday:'short',month:'short',day:'numeric'}).format(h.start)}</span>)}</div>
      <svg className="chart-svg" viewBox={`0 0 ${W} ${panelH}`} role="img" tabIndex={0} aria-description="Point or tap to read a time. Use arrow keys to move through time; Escape returns to pointer reading." onKeyDown={event=>{const direction=event.key==='ArrowRight'?1:event.key==='ArrowLeft'?-1:0;if(direction){event.preventDefault();setPinned(Math.min(end,Math.max(start,at+direction*(profile.timeIncrementMinutes||5)*60000)));}else if(event.key==='Escape')setPinned(null);}} aria-label={timing?'Dose times':`${group}, ${unit}. Exact readings are available below.`} onPointerMove={e=>{if(pinned===null)setHover(pointerTime(e));}} onClick={e=>setPinned(pinned===null?pointerTime(e):null)}>
        ...
        <g clipPath={`url(#plot-${index})`}>
          {!timing&&members.map(d=><path key={d.id} d={path(sampleTimes.map(t=>contributionForGroup(d,t,group,publishedOnly)?.value??null))} stroke={colors[visible.findIndex(v=>v.id===d.id)%colors.length]} strokeWidth="1.5" fill="none" strokeDasharray="4 4"/>)}
          {!timing&&overlays.map(({dose,values})=><path className="reference-overlay-path" key={`reference-${dose.id}`} d={path(values)} stroke={colors[visible.findIndex(item=>item.id===dose.id)%colors.length]} strokeWidth="2.3" fill="none" strokeDasharray="7 4"/>)}
          {!timing&&<><path className="total-reference-path" d={totalPaths.solid} stroke="#426a95" strokeWidth="2.7" fill="none" strokeLinejoin="round"/><path className="total-estimated-path" d={totalPaths.estimated} stroke="#426a95" strokeWidth="2.7" fill="none" strokeLinejoin="round" strokeDasharray="7 4"/></>}
      ...
      {members.length>0&&<div className="chart-legend">{!timing&&<span><i style={{background:'#426a95'}}/>{referenceOnly?'Reference illustration*':current?.complete?(unit==='ng/mL'?'Modeled total':'Illustrative sum'):hasKnownTotal(current,at)?'Known contributions*':'Unknown total'}</span>}{timelineLegend(members).map(({key,dose:d,members:entries})=><span className="legend-medication" key={key}><span className="legend-swatches" aria-hidden="true">{entries.map(entry=><i key={entry.id} style={{background:colors[visible.findIndex(v=>v.id===entry.id)%colors.length]}}/>)}</span><span className="legend-label"><MedicationName id={d.productId} name={d.productName}/><span className="legend-strength">{d.packageStrength||d.strength} {d.strengthUnit||'mg'}</span></span></span>)}</div>}
      {overlays.length>0&&<p className="chart-reference-note">* No direct data for this product. Dashed line: {overlays[0].reference.referenceLabel} reference only; excluded from the total.</p>}
      {(missingData||series.some(value=>value?.tail))&&<div className="chart-support-note">{missingData&&<span className="chart-no-data" title="One or more contributions have no supported data within this displayed range. Starred readings show the known contributions only.">* No data</span>}{series.some(value=>value?.tail)&&<span>* Dashed tail: no observed data; estimated continuation.</span>}</div>}
      </div>;
    })}
  </div>
  <div className="chart-summary">{visible.length>0&&<p className="chart-caption chart-status-legend">● Taken　◆ Planned{eventRows.length?' · No curve for unmodeled medication':''}</p>}{names.length>0&&<div className="reading-control"><time className="reading-time" dateTime={new Date(at).toISOString()} title={formatInstant(at,profile,true)} aria-label={formatInstant(at,profile,true)}>{new Intl.DateTimeFormat('en-US',{timeZone:profile.timeZone,hour:'numeric',minute:'2-digit',hourCycle:profile.timeFormat==='24h'?'h23':'h12'}).format(at)}</time><div className="reading-values">{Object.entries(totals).filter(([group])=>names.includes(group)).map(([group,g])=><span key={group} className="reading-value">{names.length>1&&<small>{group}</small>}<span><strong>{timelineReading(g,at)}</strong> <small>{g.unit}</small></span>{!g.complete&&<small>{hasKnownTotal(g,at)?'Known contributions only':'No data'}</small>}</span>)}</div>{pinned!==null&&<button className="text-button" onClick={()=>setPinned(null)}>Unpin</button>}</div>}</div>
  {unknownEarlier&&<p className="chart-caption">Earlier recorded doses have unknown contributions. Starred totals include the known contributions only.</p>}
  {omittedUnknownHistoryCount>0&&<p className="chart-caption">{omittedUnknownHistoryCount} earlier record{omittedUnknownHistoryCount===1?'':'s'} with unknown contributions {omittedUnknownHistoryCount===1?'is':'are'} not shown. Their absence does not establish that the medication has cleared.</p>}
  <div className="chart-footer"><div className="chart-footer-bar">{visible.length>0&&<button type="button" className="text-button chart-readings-toggle" aria-expanded={showReadings} aria-controls={readingsId} onClick={()=>setShowReadings(value=>!value)}><ChevronRight size={14} aria-hidden="true" style={{transform:showReadings?'rotate(90deg)':undefined}}/>Readings & details</button>}<p className="simulation-notice">Reference estimates, not measured drug levels or dose advice.{onSources&&<> <button className="text-button" onClick={onSources}>Sources & methods</button></>}</p></div>
  {visible.length>0&&<div className="chart-details" id={readingsId} hidden={!showReadings}><p className="muted">{baseline==='recorded'?'Includes recorded history. Earlier unrecorded history is unknown.':'Empty earlier history is an assumption.'} Circles = taken; diamonds = not recorded.</p>{readings.map(c=><div className="contribution" key={c.dose.id}><span><strong><MedicationName id={c.dose.productId} name={c.dose.productName}/></strong><small>{Number.isFinite(doseTimestamp(c.dose))?formatInstant(doseTimestamp(c.dose),profile,true):'Time unavailable'} · {c.dose.quantity} {c.dose.unit} × {c.dose.packageStrength||c.dose.strength} {c.dose.strengthUnit||'mg'}{Date.parse(c.dose.administeredAt)<start?' · Earlier dose':''}</small></span><span>{c.value===null?'—':`${c.value.toFixed(3)} ${c.unit}`}<small>{c.value===null?'No data':c.tail?'Estimated tail':c.evidence==='D'?'Assumptions':`Evidence ${c.evidence}`}</small></span></div>)}
    ...
    {sleeps.length>0&&<details><summary>Sleep readings</summary>{sleeps.map(s=><div key={s.start}>{[s.start,s.end].map((t,i)=><p key={t}>{i?'Wake':'Bed'} · {formatInstant(t,profile,true)}: {Object.entries(groupedTotals(visible,t,publishedOnly)).map(([name,g])=>`${name}: ${timelineReading(g,t,3)} ${g.unit}${!g.complete?(hasKnownTotal(g,t)?' (known contributions only)':' (no data)'):''}${g.tail?' (estimated tail)':''}`).join('; ')}</p>)}</div>)}<p className="muted">Target sleep schedule. Does not predict sleep quality. <button className="text-button" onClick={onProfile}>Edit schedule</button></p></details>}
  </div>}
  </div></div>;
}
