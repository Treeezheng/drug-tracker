# Drug Tracker：本机版、云版与加密边界（历史：双密码原型）

**历史记录，已被单密码 OPAQUE 方案替代。** 状态标注更新于 2026 年 9 月 14 日。以下正文保留 2026 年 9 月 13 日早期原型的说明；其中“当前”“已实现”“尚未部署”等措辞仅指当时的状态，不能用于判断现在的认证、存储或上线情况。

现行实现与边界请阅读 [README 的 Web edition](../README.md#web-edition)、[单密码 OPAQUE 协议](opaque-protocol-v1.md)和[隐私说明](../PRIVACY.md)。当前账户使用一个密码登录并解密；访客默认只在页面内存保存，可另行选择明文本地保存；账户支持有固定七天期限的设备自动解锁。源码说明与实际部署的核对方式见[安全审查记录](security-reviews.md#verification-and-limitations)。

## 原始原型记录（2026 年 9 月 13 日）
