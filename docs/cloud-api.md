...
| `GET /session` | `{ user: null }` or `{ user: { id, name } }`; no password, recovery secret, token, or username is returned. |
...
The owner header is mandatory for both vault methods. The owner comes from the authenticated session, never the body. A header mismatch fails with 409; an envelope's owner mismatch fails with 403. Logout also checks the header when supplied. Vault updates recheck the session after receiving the request body so an upload that was still in progress during logout cannot save afterward.
...
Login attempts are limited globally for this single-account service to 30 per 15 minutes, with at most two password verifications in progress. This deliberately does not trust a client-supplied forwarded IP. A 429 includes `Retry-After`. These counters are in process memory, so restarting the service resets them; reverse-proxy/network protection is an additional deployment concern. Request bodies, secrets, health values, and ciphertext are not logged by this service. Public reverse-proxy logs and hosting-provider connection metadata have separate retention settings.

## Executed checks

`node --test tests/cloud-api.test.mjs tests/vault-store.test.mjs` passes **17 tests** on this Mac using temporary synthetic databases and loopback ports. The 10 cloud API tests verify:

- Exact configuration, one-time bootstrap races, hash-only credentials, and byte-for-byte preservation of a rejected synthetic local database.
- Scoped production cookies, no-store/privacy headers, session discovery, generic failed login, and absence of plaintext/public account-management routes.
