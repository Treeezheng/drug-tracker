...
This document describes the cloud API implemented in this source tree. Test counts below are historical results from 13 September 2026; deployment and later review evidence are tracked in the [security review history](security-reviews.md). The separate entry point `server/cloud.mjs` never imports the local plaintext server, exposes none of its record endpoints, and rejects databases containing local-edition tables. See the [OPAQUE protocol](./opaque-protocol-v1.md) for registration, login and recovery messages. A source contract does not by itself verify the currently deployed version.
...
## Historical checks (13 September 2026)

The focused selections recorded then passed **41/41** SQLite/API/legacy-composition tests and **25/25** real PostgreSQL/OPAQUE/configuration tests, zero skips. Temporary synthetic databases and loopback ports were created and removed; no real user or production database was used. These selections overlap earlier results and are not a combined unique count or the current suite count.
...
The guest workspace makes no account/vault requests and cannot create Taken, symptoms or supply records. It stays in page memory by default; optional remembered guest data is unencrypted local storage. Signing in or registering does not automatically import it. After authentication, an explicit **Sync and remove local copy** choice can save it as an encrypted account simulation; those doses remain simulated. Local cleanup follows a confirmed save and preserves a newer remembered copy from another tab. See [Privacy](../PRIVACY.md) for transfer, retention and cleanup details.
