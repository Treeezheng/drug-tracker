...
`csvString(doses,profile,from,to,checkins=[])` and `downloadCsv(doses,profile,from,to,checkins=[])` now include actual doses and independent check-in rows in chronological order. Existing 26 columns retain their order; appended columns are `event_type`, `symptoms`, `checkin_time_utc`, `checkin_original_timezone`, `checkin_original_date`, `legacy_focus`, and `legacy_sleep_quality`. Symptom rows have no medication/product/dose amounts assigned to them. Timestamped observations use the report time zone for date filtering; date-only legacy observations keep their original date and leave the exact time blank. Symptom-only dates export even when no doses exist. Tags are canonical IDs, and the disclosure explicitly avoids attributing symptoms to medication. The current PDF remains an actual-administration report.

For a filesystem backup, stop the server before copying the SQLite file, or use a proper SQLite online backup method. Copying only the main file while the server is running can omit writes held in its WAL. Keep backups private. CSV/PDF reports are generated by the frontend from authenticated data; they must preserve units and exclude simulations from actual consumption totals.
