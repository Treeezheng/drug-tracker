...
同一已开认证会话会拒绝已观察到的更低 revision；新浏览器/刷新后没有独立可信最新版本锚点，无法彻底阻止恶意服务器回放、删除或拒绝服务。会话/账户标识、IP、访问时间、密文大小仍为可见元数据。访客默认仅内存，18岁声明后主动 Remember 的模拟草稿为明文 localStorage；本机 SQLite、导出的 CSV/PDF/JSON 也不属于云密文承诺。
