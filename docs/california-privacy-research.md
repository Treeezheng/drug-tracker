...
核对日期：**2026-09-13**。范围为个人开源 Drug Tracker：访客默认仅在页面内存演算，主动选择记住才明文保存在该浏览器，可另外注册；账户药物、症状、库存等由浏览器加密后保存在运营者服务器。本文是条件性法律研究，不是合规认证或针对运营者全部事实的法律意见；向他人提供健康记录托管时，应让熟悉加州健康隐私的律师确认适用范围。程序、政策和生产配置的核验与本法律研究分别记录。
...
对本项目的实际事项：分开说明访客可选 localStorage 明文、账户密文、账号与连接元数据、主机/备份、导出、删除流程和可联系的运营者；不能只写“不卖数据”。当前政策已明确站内显著通知重大变更的方法，并披露 Heroku NEL；无广告脚本不等于无第三方诊断报告。**本研究不宣称当前 Privacy 已满足 CalOPPA**。用户记录查阅/更正流程虽已提供，§22575(b)(2) 的原文条件是“如运营者维持这类流程”，不宜把它概括为该条单独强制所有网站建立同样的流程。
