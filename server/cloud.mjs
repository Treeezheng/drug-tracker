...
function staticBuild(distDir) {
  ...
  for (const name of ['privacy.html', 'terms.html', 'chart-guide.html', 'LICENSE', 'THIRD_PARTY_NOTICES.txt', 'robots.txt', 'llms.txt', 'favicon.svg', 'favicon.ico', 'apple-touch-icon.png', 'icon-192.png', 'icon-512.png', 'site.webmanifest', 'build-info.json']) {
    try { capture(name); } catch (error) { if (error.code === 'ENOENT') continue; throw error; }
    if (name === 'privacy.html' || name === 'terms.html' || name === 'chart-guide.html') documentPages.set(name, files.get(name));
  }
  ...
  return { index: files.get('index.html'), files, documentPages };
}
    ...
    try {
      ...
      if (build && ['GET', 'HEAD'].includes(req.method)) {
        ...
        if (path.startsWith('/drug/')) {
          ...
          if (build.documentPages.has(relative)) {
            const documentPage = build.documentPages.get(relative);
            ...
            res.writeHead(200, { 'Content-Type': MIME['.html'], 'Content-Length': documentPage.length });
            return res.end(req.method === 'HEAD' ? undefined : documentPage);
          }
          ...
          // Only the captured edition-checked HTML and explicitly allowed documentation pages are served.
