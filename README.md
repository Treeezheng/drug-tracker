...
**Hide**, sign-out and disabling automatic unlocking revoke and request deletion of this browser’s saved key; password/key changes invalidate old copies. Leaving the page clears memory while preserving an enabled key until expiry. Expired entries are cleaned up when read, not by a guaranteed deletion timer. Turning the setting off requires a password again after reload; enabling it from Security settings requires password confirmation. Anyone using this browser profile or scripts on the same origin may use the saved key; it is not a claim of hardware-backed protection. Reloading, hiding or detecting expiry discards unsaved account entries. The separate, unencrypted guest workspace can remain in the browser. Concurrent saves are checked against the vault revision; conflicts do not silently replace another device’s records.
