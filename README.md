...
The chart groups medications by **the same modeled compound and concentration unit**. Concerta and Ritalin share the methylphenidate view: each formulation keeps its own curve, and compatible contributions add to the estimated total, including available earlier doses. Adderall remains separate from methylphenidate. Overlapping amphetamine models share a group, with a second selector keeping d- and l-amphetamine totals separate. Unsupported contributions remain unknown, and relative illustrations do not enter physical concentration totals. The selector switches curves, readings, sources and prior-dose carryover together; grouping does not establish safe coadministration or predict interactions.
...
References in [the registry](src/lib/pk-references.ts) use FDA/DailyMed single-dose parameters, rounded published figures or explicitly constructed landmarks. Immediate-, extended- and delayed-release formulations retain their own models within a compatible concentration group. Interpolation, dose scaling, fitted absorption, generic reference transfer and terminal tails are estimates. The original record's evidence is not upgraded just because a reference can be drawn. Concerta 18 mg's reconstructed group trace and Ritalin IR 10 mg's parameter-based curve are also references, not individual measurements. Formula and Sources & methods identify each formulation's population, dose basis and numerical limitations.
...
**Recording only** marks catalog entries without an implemented concentration model in the medication chooser, dose options and record labels. These entries remain available for recording; the chooser explains the marker once. Reference-based curves count as supported for this label even when their evidence grade is D. Individual dose validation and unknown contributions still apply.
...
This source implements the **single-password OPAQUE account flow**. The [two-password prototype](docs/editions-and-encryption.md) and [candidate review dated 13 September 2026](docs/opaque-release-review-2026-09-13.md) are historical records. The [current protocol](docs/opaque-protocol-v1.md) describes the implementation; [security review history](docs/security-reviews.md#verification-and-limitations) explains how source, CI, signed manifests and live assets are checked. Release-specific results belong in the corresponding pull request’s verification record; this README alone does not prove which version is live.
