...
const render=(dose:Dose,favorites?:Favorite[])=>renderToStaticMarkup(createElement(DoseEditor,{dose,index:0,profile,favorites,onChange:()=>{throw Error('Rendering cannot change a dose.');}}));
...
test('a record retains its original package after favorites change, including custom and unavailable products',()=>{
  ...
  assert.match(customHtml,/aria-label="Dose 1 custom strength in mg"[^>]*value="7.25"/);
...
test('grouped favorites expose each selected strength while retaining its product identity and release system',()=>{
...
test('a grouped strength selection explicitly changes product snapshots without rounding quantity or transferring reference evidence',()=>{
  ...
  const genericChoice=doseStrengthChoices(original,favorites).find(choice=>choice.packageStrength==='5')!;
  ...
  const returned=selectDoseStrength(generic,doseStrengthChoices(generic,favorites).find(choice=>choice.packageStrength==='10')!,'UTC');
...
test('choosing a medication group applies its first favorite once, while later strength choices stay selected',()=>{
