...
We use account and session information to authenticate you, keep accounts separate and operate the service. Encrypted snapshots let you save and retrieve your records. Decrypted account records stay in memory while unlocked; there is no persistent account-record cache or offline queue. The app does not save your password or OPAQUE export secret in browser storage. The separate guest workspace may still be present.

**Auto-unlock on this device for 7 days** is a visible choice on sign-in and registration forms, selected by default. It takes effect only after you submit and successfully authenticate with your password; you can uncheck it before submitting. You can also enable it in Security settings after confirming your password through OPAQUE, or turn it off there. When enabled, this browser stores an encrypted copy of the data decryption key and a non-extractable browser wrapping key in a dedicated IndexedDB store, with the account identifier, a fingerprint of its encrypted key wrapper and creation/expiry metadata. A separate local-storage marker contains only a public random revocation value. Neither key is sent to the server.

The saved unlocking key has a fixed **7-day expiry from successful password authentication**. Returning to the site or automatically unlocking does not extend it. Automatic unlocking requires an internet connection, a currently valid server session and a fresh encrypted snapshot whose owner and key wrapper match the saved key. A shorter or revoked server session ends access sooner. **Hide**, sign-out and disabling automatic unlocking revoke this browser’s saved key and request its deletion; expired or invalid entries are cleaned up when read, not by a guaranteed physical-deletion timer. If browser storage is unavailable, automatic unlocking may be unavailable. Anyone who can use this browser profile may be able to open your records during this period. Scripts on this site can use the browser key even though it cannot be exported through the browser key API; this feature is not a claim of hardware-backed protection.

## Cookies, connections and providers

The hosted service uses an essential, HttpOnly session cookie to keep you signed in. Sessions have a **7-day maximum lifetime from creation**, without sliding renewal; sign-out, deletion or other session invalidation may end access sooner. Older sessions retain any earlier expiry; sessions exceeding the maximum are capped from their original creation time on server startup. Blocking the cookie prevents account access. The site also uses the browser unlock storage and optional guest storage described above.
...
Saving account changes requires an internet connection. **Hide** clears the unlocked account workspace without a connection and revokes this browser’s saved unlocking key. The page requires password authentication again after its fixed **7-day unlock period**, even if you keep using it. An open account page attempts periodic server-session checks and checks when it regains focus. A suspended browser can delay timers; the deadline is checked again when you return or interact. Offline pages cannot confirm remote session revocation. Reloading, closing the page, hiding records or detecting expiry can discard unsaved changes. Leaving the page clears its in-memory workspace while preserving an enabled, unexpired browser unlock record. Session revocation does not instantly erase data already displayed in another browser.
...
**Keeping the recovery key safe is your responsibility.** Before continuing after a new key is shown, you must confirm that you have saved it. This records your acknowledgment; it cannot prove that a copy was successfully saved or remains accessible. A downloaded recovery-key file is unencrypted. Anyone with the complete key may access your account and records. The operator does not keep a usable copy and cannot recover your password or key for you. If both are lost, a browser that can still unlock may let you view or export current records until its access ends. Without either credential or an accessible unlocked browser, records cannot be recovered through this service; there is no email or customer-support reset that bypasses the credentials.

While signed in and unlocked, **Settings → Account → Security** lets you change your password, replace the recovery key or sign out all devices, after confirming your current password through OPAQUE. **Change password** re-protects the same data key, so the existing recovery key continues to work. **Replace recovery key** creates a new data key, re-encrypts current records and supplies a new recovery key. Password changes and key replacement revoke old sessions and create a new current session; all-device sign-out ends the current session too. Password or key changes invalidate older browser unlock copies. These actions do not erase previously copied ciphertext, old key wrappers, downloads or backups. An old key can still unlock a copy made before rotation.
