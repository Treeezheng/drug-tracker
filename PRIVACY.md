...
Saving account changes requires an internet connection. **Lock** clears the unlocked account workspace without a connection and revokes this browser’s saved unlocking key. The page requires password authentication again after its fixed **7-day unlock period**, even if you keep using it. An open account page attempts periodic server-session checks and checks when it regains focus. A suspended browser can delay timers; the deadline is checked again when you return or interact. Offline pages cannot confirm remote session revocation. Reloading, closing the page, locking records or detecting expiry can discard unsaved changes. Leaving the page clears its in-memory workspace while preserving an enabled, unexpired browser unlock record. Session revocation does not instantly erase data already displayed in another browser.
